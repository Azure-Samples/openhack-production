...
Primary application used within single sign-on flows from frontend.
...
# Identity Providers
Azure AD B2C supports many popular identity providers out of the box. Additional iIdentity providers can also be configured as long as it supports OpenID Connect (OIDC).
...
# User Flows
User flows support various common user interactions including sign-in, registration, profile editing, password reset & more. Additionally they support configuration to customize the user attribues and claims required by your applications for requested id and access tokens.

![Azure AD B2C User Flows](images/azure-b2c-user-flows.png)

The Urlist application primarility utilizes a combined sign-in / sign-up flow.
...
After running through the user flow and signing in with a configured identity provider you will be redirected back to the redirect url chosed during your configuration.  Assuming the frontend application has been configured correctly you should now be logged in with your selected account.

![Signed in to application](images/azure-b2c-signed-in.png)

# Integrating with your application code
When integrating your applications to leverage Azure AD B2C you will likly be asked for 1 ore more configuration elements. The following configuration elements are commonly used within many applications.
...
## Tenant Domain
The fully qualified tenant domain host name.
...
## Other
Some applications may require a concatentation of some of the above elements.  For example the full Authority URL required for integrationg ROPC from a console application to the backend REST API.
